...
function generateMatchReason(matchedKeywords: string[]): string {
  ...
  return `Matched ${matchedKeywords.length} keywords including: ${matchedKeywords.slice(0, 3).map(k => `"${k}"`).join(', ')}`
}
 ...
 */
const aiMatchCache = new Map<string, {
 ...
 */
const heatCheckCache = new Map<string, {
