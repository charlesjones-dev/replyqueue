  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (response.status === 403) {
        throw new OpenRouterError(
          'Access denied. Your API key may not have access to this resource.',
          403,
          false
        )
      }
...
export async function chatCompletion(
  ...
  console.log(
    `${LOG_PREFIX} Chat completion successful, tokens used: ${response.usage?.total_tokens ?? 'unknown'}`
  )
...
export async function fetchModels(apiKey: string): Promise<OpenRouterModel[]> {
  ...
  const response = await makeRequest<{ data: RawOpenRouterModel[] }>(
    '/models',
    { method: 'GET' },
    apiKey
  )
  ...
  if (!forceRefresh) {
    ...
    if (cached && isCacheValid(cached)) {
      const minutesRemaining = Math.round(
        (cached.fetchedAt + cached.ttl - Date.now()) / 1000 / 60
      )
